...
// === Toast Alert Wrapper (V1.5) ===
// Ersetzt alert() durch moderne Toast-Notifications
(function() {
  // Warte bis Toast-System geladen ist
  const originalAlert = window.alert;
  
  window.alert = function(message) {
    // Fallback auf originales alert falls toast noch nicht da
    if (!window.toast || !window.toast.info) {
      return originalAlert(message);
    }
    
    // Bestimme Toast-Typ basierend auf Message-Content
    const msg = String(message || '');
    const lowerMsg = msg.toLowerCase();
    
    if (lowerMsg.includes('fehler') || lowerMsg.includes('fehlgeschlagen') || lowerMsg.includes('error')) {
      window.toast.error(msg);
    } else if (lowerMsg.includes('warnung') || lowerMsg.includes('ungültig')) {
      window.toast.warning(msg);
    } else if (lowerMsg.includes('erfolg') || lowerMsg.includes('gespeichert') || lowerMsg.includes('erstellt') || lowerMsg.includes('gelöscht') || lowerMsg.includes('gesendet')) {
