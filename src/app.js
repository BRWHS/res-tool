...
window.addEventListener('keydown', (e)=>{ if (e.key === 'Escape') window.closeModal(); });
  ...
  // Minimaler Submit-Controller: robust gegen Re-Renders
let submittingReservation = false;

function validateReservationDraft(d){
  const e=[];
  if(!d.hotel_code) e.push('Hotel fehlt.');
  if(!d.arrival||!d.departure) e.push('An- und Abreise fehlen.');
  if(d.arrival && d.departure && d.arrival > d.departure) e.push('Abreise vor Anreise.');
  if(!d.guest_last_name) e.push('Gast Nachname fehlt.');
  if(!d.category) e.push('Zimmerkategorie fehlt.');
  if(!d.rate_name) e.push('Rate fehlt.');
  if(d.rate_price==null||isNaN(Number(d.rate_price))) e.push('Preis ungültig.');
  if(!d.cc_holder||!d.cc_last4||!d.cc_expiry) e.push('Zahlungsdaten unvollständig.');
  return e;
}

function buildReservationPayload(d){
  return {
    hotel_code:d.hotel_code, hotel_name:d.hotel_name,
    arrival:d.arrival, departure:d.departure,
    guest_first_name:d.guest_first_name||null, guest_last_name:d.guest_last_name,
    email:d.email||null, phone:d.phone||null,
    category:d.category, rate_code:d.rate_code||null, rate_name:d.rate_name,
    rate_price:Number(d.rate_price), notes:d.notes||null,
    cc_holder:d.cc_holder, cc_last4:d.cc_last4, cc_expiry:d.cc_expiry,
    status:'active', created_at:new Date().toISOString()
  };
}

function toast(msg,type='info'){
  console[type==='error'?'error':'log']('[res-tool]',msg);
  const el=document.getElementById('appToast');
  if(el){ el.textContent=msg; el.className=`toast show ${type}`; setTimeout(()=>el.className='toast',2500); }
}

// Delegierter Click-Handler (überlebt Re-Renders)
document.addEventListener('click', async (ev)=>{
  const t=ev.target;
  if(!t || t.id!=='btnReservationCreate') return;
  ev.preventDefault();

  if(submittingReservation) return;
  submittingReservation=true;

  const btn=t, label=btn.textContent;
  btn.disabled=true; btn.textContent='Wird erstellt …';

  try{
    const d=window.reservationDraft||{};
    const errs=validateReservationDraft(d);
    if(errs.length){ toast('Bitte prüfen: '+errs.join(' '),'error'); return; }

    const payload=buildReservationPayload(d);
    const { data, error } = await supabase.from('reservations').insert([payload]).select().single();
    if(error){ console.error(error); toast('Fehler beim Speichern der Reservierung.','error'); return; }

    toast('Reservierung erfolgreich erstellt.');
    window.reservationDraft={};
    if(typeof closeReservationWizard==='function') closeReservationWizard();
    if(typeof refreshReservationsTable==='function') refreshReservationsTable();
    if(typeof refreshKpis==='function') refreshKpis();

  }catch(e){
    console.error(e); toast('Unerwarteter Fehler beim Erstellen.','error');
  }finally{
    submittingReservation=false; btn.disabled=false; btn.textContent=label;
  }
});
