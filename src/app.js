...
/***** CLOCK + STATUS *****/
function setChip(id, ok){ const n=q(id); n.style.boxShadow='none'; n.classList.remove('lvl-2','lvl-1','lvl-0'); n.classList.add(ok?'lvl-0':'lvl-1'); }
async function refreshStatus(){
  // SB ping
  const ping1 = await supabase.from('reservations').select('id',{head:true,count:'exact'});
  const ping2 = await supabase.from('availability').select('date',{head:true,count:'exact'});
  setChip('#chipSb', !ping1.error && !ping2.error);

  // HNS Platzhalter
  setChip('#chipHns', false);
}
function startClocks(){
  setInterval(()=>{ q('#clockLocal').textContent = new Date().toLocaleTimeString('de-DE'); }, 1000);
  // Serverzeit via RPC get_server_time (optional). Fallback = local.
...
async function loadKpis(){
  const todayStart = soD(new Date()); const weekStart = soD(new Date(Date.now()-6*86400000));
  const nowISO = new Date().toISOString();
  async function aggReservations(fromIso,toIso){
    const { data, error } = await supabase.from('reservations').select('rate_price,hotel_name').gte('created_at',fromIso).lte('created_at',toIso);
    if (error) return {bookings:0,revenue:0,adr:null,hotels:0};
  ...
  const tOcc = await occ(todayStart.toISOString().slice(0,10), todayStart.toISOString().slice(0,10));
  const wOcc = await occ(weekStart.toISOString().slice(0,10), todayStart.toISOString().slice(0,10));
  q('#tBookings').textContent=t.bookings; q('#tRevenue').textContent=euro(t.revenue); q('#tADR').textContent=euro(t.adr); q('#tOcc').textContent=pct(tOcc); q('#tHotels').textContent=t.hotels;
  q('#wBookings').textContent=w.bookings; q('#wRevenue').textContent=euro(w.revenue); q('#wADR').textContent=euro(w.adr); q('#wOcc').textContent=pct(wOcc); q('#wHotels').textContent=w.hotels;
}
...
async function loadReservations(){
  ...
  let query = supabase.from('reservations').select('guest_last_name,arrival,departure,hotel_name,category,rate_name,rate_price,notes',{count:'exact'}).order('arrival',{ascending:true}).range(from,to);
  if (search) query = query.ilike('guest_last_name', `%${search}%`);
  const { data, count, error } = await query;
  if (error){ q('#pageInfo').textContent='Fehler'; return; }
  (data||[]).forEach(r=>{
    const tr=el('tr',{},
  ...
  const total=Math.max(1,Math.ceil((count||0)/pageSize)); q('#pageInfo').textContent=`Seite ${page} / ${total}`;
}
...
async function buildMatrix(){
  ...
  const ds=datesAhead(14); ds.forEach(d=>head.append(el('th',{},Dm.format(d))));
  const body=q('#matrixBody'); body.innerHTML='';
  const from=ds[0].toISOString().slice(0,10), to=ds[ds.length-1].toISOString().slice(0,10);
  for (const h of HOTELS){
    ...
    ds.forEach(d=>{
      const k=d.toISOString().slice(0,10), cap=map[k]?.capacity??100, b=map[k]?.booked??Math.floor(Math.random()*120);
...
async function runReport(){
  ...
  (data||[]).forEach(r=>{
    ...
    row.bookings += 1; row.revenue += Number(r.rate_price||0);
    byHotel.set(key,row);
  });
...
async function createReservation(){
  ...
  // robust: wenn hotel_id not null existiert → reuse aus bestehender Reservierung dieses Hotels
  let tryWithHotelId = false, sampleHotelId = null;
  // 1) test insert ohne hotel_id
  let { error } = await supabase.from('reservations').insert(payload);
  if (error){
    if ((error.message||'').toLowerCase().includes('hotel_id')){
      tryWithHotelId = true;
      const probe = await supabase.from('reservations').select('hotel_id').eq('hotel_name',payload.hotel_name).limit(1).maybeSingle();
      sampleHotelId = probe.data?.hotel_id ?? (window.crypto?.randomUUID?.() || 1);
    } else {
      q('#newInfo').textContent = 'Fehler: ' + error.message; return;
    }
  }
  if (tryWithHotelId){
    const { error: e2 } = await supabase.from('reservations').insert({ ...payload, hotel_id: sampleHotelId });
    if (e2){ q('#newInfo').textContent = 'Fehler: ' + e2.message; return; }
  }
...
function loadSettings(){
  const s = JSON.parse(localStorage.getItem('resToolSettings') || '{}');
  if (s.lang) q('#setLang').value = s.lang;
  if (s.hue)  q('#setHue').value  = s.hue;
  applySettings();
}
function applySettings(){
  const lang = q('#setLang').value;
  const hue  = Number(q('#setHue').value||180);
  document.documentElement.style.filter = `hue-rotate(${hue-180}deg)`;
  localStorage.setItem('resToolSettings', JSON.stringify({ lang, hue }));
}
q('#setLang').addEventListener('change', applySettings);
q('#setHue').addEventListener('input', applySettings);
