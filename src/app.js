...
/***** MODALS (sharp background, no blur) *****/
...
window.addEventListener('keydown', (e)=>{ if (e.key==='Escape') { qa('.modal').forEach(m=>m.style.display='none'); backdrop.style.display='none'; }});
...
async function refreshStatus(){
  ...
  setChip(q('#chipHns'), false); // placeholder
}

/***** KPI LOAD (robust, with fallbacks) *****/
async function loadKpis(){
  ...
  async function aggReservations(fromIso, toIso){
    // prefer created_at; if missing, fallback to arrival date filter
    ...
    const hotels   = new Set(rows.map(r=>r.hotel_name).filter(Boolean)).size;
  ...
  async function occ(fromDate, toDate){
    ...
    const { data, error } = await supabase
      .from('availability').select('capacity, booked')
...
async function createReservation(){
  ...
  if (error && (error.message||'').toLowerCase().includes('hotel_id')) {
    // Try again with a sample hotel_id if schema requires it
